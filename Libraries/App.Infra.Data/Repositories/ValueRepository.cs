...
namespace App.Infra.Data.Repositories
{
    ...
    public class ValueRepository : IValueRepository
    {
        ...
        public ValueRepository(AppDbContext context,IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
            _httpContextAccessor = httpContextAccessor;
        }
        public async Task<Value> GetItemById(int Id)
        {
        ...
        {
            var dbObj = await _context.Value.
        ...
        {
            var dbObj = await _context.Value.Where(x => x.Id == id).FirstOrDefaultAsync();
            if (dbObj == null)
            {
                return false;
            }
        ...
        public async Task SaveItem(Value obj)
        {
            if (obj.Id == 0)
            {
                obj.IsDeleted = false;
                ...
                await _context.Value.AddAsync(obj);
            }
            else
            {
        ...
        public async Task<Tuple<IList<Value>, int>> LoadItemsData(string Search, int StatusId,  int jtStartIndex = 0,
            int jtPageSize = 10, string order = null, string orderDir = null, int languageId = 0)
        {
           var AllListCount = 0;
            var dataQuery = _context.Value
            ...
            return new Tuple<IList<Value>, int>(await result.Skip(jtStartIndex).Take(jtPageSize).ToListAsync(), AllListCount);
