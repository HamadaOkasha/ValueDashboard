...
namespace App.Infra.Data.Repositories
{
    ...
    public class OrganizationRepository : IOrganizationRepository
    {
        ...
        public OrganizationRepository(AppDbContext context,IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
            _httpContextAccessor = httpContextAccessor;
        }
        public async Task<Organization> GetItemById(int Id)
        {
        ...
        {
            var dbObj = await _context.Organization.
        ...
        {
            var dbObj = await _context.Organization.Where(x => x.Id == id).FirstOrDefaultAsync();
            if (dbObj == null)
            {
                return false;
            }
        ...
        public async Task SaveItem(Organization obj)
        {
            if (obj.Id == 0)
            {
                obj.IsDeleted = false;
                ...
                await _context.Organization.AddAsync(obj);
            }
            else
            {
        ...
        public async Task<Tuple<IList<Organization>, int>> LoadItemsData(string Search, int StatusId,  int jtStartIndex = 0,
            int jtPageSize = 10, string order = null, string orderDir = null, int languageId = 0)
        {
           var AllListCount = 0;
            var dataQuery = _context.Organization
            ...
            return new Tuple<IList<Organization>, int>(await result.Skip(jtStartIndex).Take(jtPageSize).ToListAsync(), AllListCount);
