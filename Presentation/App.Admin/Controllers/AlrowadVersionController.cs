...
{
    //
    public class AlrowadVersionController : BaseController
    {
        private readonly IAlrowadVersionSevices _AlrowadVersionSevices;
        ...
        [HttpPost]
        public async Task<IActionResult> AddEdit(AlrowadVersionModel model)
        {
            await _AlrowadVersionSevices.Save(model);
            TempData["successMsg"] = _localeStringResourceSevices.GetResource("SuccessfullyMsg");
            return RedirectToAction("Index", "AlrowadVersion");
        }
        [HttpPost]
        public async Task<IActionResult> Delete(int id)
        {
            var result = await _AlrowadVersionSevices.Delete(id);
            return Json(result);
        }
        [HttpPost]
        public async Task<IActionResult> Status(int id, bool status)
        {
            var result = await _AlrowadVersionSevices.ChangeStatus(id, status);
            return Json(result);
        }
        ...
        public async Task<JsonResult> LoadData(string Search, int StatusId)
        {
            ...
            // int totalRecords;
            // Loading.
            var data = await _AlrowadVersionSevices.LoadData(Search, StatusId,  startRec,
